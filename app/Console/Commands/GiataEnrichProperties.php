...
class GiataSyncProperties extends Command
{
    ...
    protected $description = 'Sincroniza GIATA properties. Si no se indica --provider, recorre una lista fija de providers. Filtra por activos del provider base (--only-active). Con --save-codes guarda los codes activos de TODOS los providers (o los indicados en --providers) para cada propiedad.';
    ...
    public function handle(): int
    {
        ...
        foreach ($providerCodes as $baseProviderCode) {
            ...
            if (!$providerRow) {
                ...
                // Si quieres seguir con el siguiente provider, usa "continue" en lugar de "return"
                return self::FAILURE;
            }
                                ...
                                if ($detailXml && isset($detailXml->property)) {
                                    ...
                                    if (!empty($allCodes)) {
                                        ...
                                        foreach ($allCodes as $c) {
                                            ...
                                            // 🔎 Comprobar si YA existe ese provider + code_value en la BD
                                            $exists = GiataPropertyCode::where('provider_id', $provRow->id)
                                                ->where('code_value', $c['code_value'])
                                                ->exists();

                                            if ($exists) {
                                                // Opcional: log rápido
                                                // $this->output->writeln("\n[SKIP] Code '{$c['code_value']}' para provider {$c['provider_code']} ya existe. No se inserta.");
                                                continue; // saltamos este code, no lo añadimos al payload
                                            }
