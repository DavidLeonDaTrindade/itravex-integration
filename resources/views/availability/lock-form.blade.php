    ...
    <div class="max-w-6xl mx-auto px-4 py-8">
        ...
        <div class="container mx-auto px-4 py-6 bg-white rounded-lg shadow">
            ...
            <div class="mt-10 p-6 bg-yellow-100 border border-yellow-400 rounded-md shadow-sm">
                ...
                <form method="POST" action="{{ route('availability.close') }}" class="mt-4 space-y-4">
                    @csrf

                    <fieldset class="bg-white rounded-md p-4 border">
                        <legend class="font-semibold text-gray-800">Datos de contacto de la reserva</legend>

                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                <label class="block text-sm text-gray-700 mb-1" for="percon_nombre">Nombre</label>
                                <input id="percon_nombre" name="percon[nombre]" type="text" required
                                    class="w-full border rounded px-3 py-2"
                                    value="{{ old('percon.nombre') }}">
                            </div>

                            <div>
                                <label class="block text-sm text-gray-700 mb-1" for="percon_priape">Primer apellido</label>
                                <input id="percon_priape" name="percon[priape]" type="text" required
                                    class="w-full border rounded px-3 py-2"
                                    value="{{ old('percon.priape') }}">
                            </div>

                            <div>
                                <label class="block text-sm text-gray-700 mb-1" for="percon_tel">Teléfono</label>
                                <input id="percon_tel" name="percon[tel]" type="tel" required
                                    inputmode="tel" pattern="[0-9+\s\-()]{6,}"
                                    class="w-full border rounded px-3 py-2"
                                    value="{{ old('percon.tel') }}">
                            </div>
                        </div>
                    </fieldset>
