        ...
        */
        ...
        <div id="top" class="max-w-7xl mx-auto px-6 py-10">
            ...
            <div class="flex items-center justify-between mb-6">
                ...
                {{-- DEBUG opcional --}}
                {{-- <div class="text-xs text-slate-500">len(firstResponse): {{ strlen($firstResponse ?? '') }}
            </div> --}}
            ...
            <div class="flex items-center gap-2">
                ...
                <button id="btn-show-xml"
                    class="inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700 shadow-sm hover:bg-slate-50 hover:border-slate-400 transition">
                ...
                {{-- Botón ver respuesta XML (AHORA SÍ, BOTÓN EXPLÍCITO) --}}
                @if(!empty($firstResponse))
                <button id="btn-show-response"
                    class="inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700 shadow-sm hover:bg-slate-50 hover:border-slate-400 transition">
                    <svg class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
        ...
        {{-- MODAL: Petición XML --}}
        @if(!empty($firstPayload))
        <div id="xml-modal" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50">
            <div class="bg-white rounded-xl shadow-lg max-w-3xl w-full p-6 relative">
                <button id="btn-close-xml" class="absolute top-3 right-3 text-gray-500 hover:text-gray-800">✖</button>
        ...
        {{-- MODAL: Respuesta XML --}}
        @if(!empty($firstResponse))
        <div id="response-modal" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50">
            <div class="bg-white rounded-xl shadow-lg max-w-3xl w-full p-6 relative">
                <button id="btn-close-response" class="absolute top-3 right-3 text-gray-500 hover:text-gray-800" type="button">✖</button>
                <h2 class="text-lg font-semibold text-slate-800 mb-4">📥 XML de la respuesta</h2>
                <pre class="bg-slate-50 border rounded p-3 text-xs overflow-auto max-h-[70vh] text-slate-700">{{ $firstResponse }}</pre>
            </div>
        </div>
        @endif
                                                ...
                                                <li
                                                    ...
                                                    <div class="room-right">
                                                        ...
                                                        <form method="GET"
                                                            ...
                                                            @php
                                                            // 1) Si vienes de multi, usa la 1ª ocupación del array
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                var btnResp = document.getElementById('btn-show-response');
                var modalResp = document.getElementById('response-modal');
                var closeResp = document.getElementById('btn-close-response');

                if (btnResp && modalResp) {
                    btnResp.addEventListener('click', () => modalResp.classList.remove('hidden'));
                    if (closeResp) closeResp.addEventListener('click', () => modalResp.classList.add('hidden'));
                    modalResp.addEventListener('click', e => {
                        if (e.target === modalResp) modalResp.classList.add('hidden');
                    });
                }
            });
        </script>
